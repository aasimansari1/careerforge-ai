    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden w-1/2 flex-col justify-between gradient-bg p-12 text-white lg:flex">
        ...
        <div>
          ...
          <p className="mt-3 text-lg text-white/80">Continue building your perfect resume and land your dream job.</p>
          <div className="mt-8 space-y-3">
            {["30+ AI-powered tools", "40+ ATS-optimized templates", "Real-time resume scoring"].map((f) => (
              <div key={f} className="flex items-center gap-2 text-sm text-white/90">
                <div className="h-1.5 w-1.5 rounded-full bg-white" /> {f}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex w-full flex-col items-center justify-center px-6 lg:w-1/2">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-border bg-background py-3 pl-10 pr-10 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20"
                />
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Don&apos;t have an account? <Link href="/signup" className="font-medium text-primary hover:underline">Sign up free</Link>
          </p>
        </motion.div>
      </div>
    </div>
